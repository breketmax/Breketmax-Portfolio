...
import xakaton from "./xakaton.png";
...
export const API = {
  ...
  feedback:[
    {
      review:"Гибкий, въедливый, активным молодой специалист.</br>" +
          "Нуждается в присмотре (как в поощрении, так и в твёрдой руке).</br>" +
          "Позитивный, яркий - может быть основой \"настроения команды\".</br>" +
          "Договороспособный, готов к работе над ошибками, готов говорить о планах и проблемах",
      name:"Александр",
      post:"Тимлид"
    },
    {
      review:"1. Вовремя эскалирует проблемы, и приносит несколько вариантов решения, что позволяет быстро среагировать и принять меры</br>" +
          "2. Находит дыры в постановках задач и самостоятельно инициирует их закрытие привлекая Аналитика или Дизайнера</br>" +
          "3. Хороший навык оценки и декомпозиции задач. Умеет составить себе \"чек-лист\" для разработки с оценкой каждого пункта, что в дальнейшем удобно мониторить</br>" +
          "4. При необходимости передать часть задачи другому специалисту достаточно подробно описывает, что сделано, что осталось и куда обратить внимание в коде. Однако в описании использует просторечия, что иногда может неверно передать посыл передаваемой работы",
      name:"Артем",
      post:"Менеджер"
    },
    {
      review:"Быстро переключается между задачами.</br>" +
          "Быстро вникает в суть задачи/бага, может аргументировать сколько времени займет и почему столько. Хороший навык декомпозиции</br>" +
          "Отзывчивый, при наличии вопросов всегда поможет.</br>" +
          "Если у него возникают вопросы - он подойдет и спросит, инициирует общение. Однозначно лайк.</br>" +
          "Очень позитивыный.",
      name:"Юлия",
      post:"Менеджер"
    },
    {
      review:"Огненный разраб, с потенциалом роста.</br>" +
          "Главное не оставлять его без присмотра, и следить за тем чтобы он понимал, как и почему важна его деятельность, иначе энтузиазм затухает.</br>" +
          "Легко берёт ответственность как за коллег, так и за результат работы, но только если хорошо проработать договорённость и взаимные ожидания.</br>" +
          "Подтянул все рекомендации от старших специалистов.</br>" +
          "Моя оценка совместной жизни 5 из 5",
      name:"Александр",
      post:"Тимлид"
    },
    {
      review:"1. Знает все тонкости в коде и архитектуре проекта со стороны фронта.</br>" +
          "2. Помогает разбираться другим разработчикам в легаси.</br>" +
          "3. Стал проявлять инициативу в структуризации работы фронтов (во время хотфиксов быстро составил таблицу с багами, собрал вокруг себя фронтов и распределил между всеми задачи)</br>" +
          "4. Ответственный, переживает за продукт. Если необходимо задержаться по понятным причинам и доделать работу, то доделает или договорится со всеми о решении вопросов разработки так, чтобы это не блокировало работу другим членам команды",
      name:"Сергей",
      post:"Менеджер"
    },
    {
      review:"Показал себя как хороший исполнитель, знающий кодовую базу. Поработал над отчетами перед командой на дейли митинге. Заводит задачи и трекает время в них, если нет форс мажоров. Периодически сбивается по активным задачам с командой. Старается быть в курсе последних изменений проекта, аналитики, требований и приоритетов. Неплохо выполняет роль старшего на проекте. Подробно декомпозирует задачи. Консультирует по написанному ранее коду, хорошо знаком с API проекта. Учится делегированию задач, есть значительный прогресс. Хорошо разбирается в теории. Принимал участие в составлении вопросов для повышения квалификации младшего специалиста.",
      name:"Алексей",
      post:"Старший специалист"
    },
    {
      review:"4,7 из 5 оценка командной удовлетворённости</br>" +
          "</br>" +
          "Что отмечают:</br>" +
          "Позитивный в работе и в общении. Не создаёт никому напрягов и разряжает атмосферу.</br>" +
          "Всех фронтов погружал в проект и консультировал беков - очень большая заслуга окончания блока работ.</br>" +
          "Улучшилось качество выполняемых обязательств и в работе и в орг.вопросах. Улучшился в вопросах ответственности.",
      name:"Команда",
      post:"Все подряд"
    },
  ],
  timeline:[
    ...
    {
      img:pixlPark,
      data:"Апрель  2021 - Сентябрь 2022",
      title:"PixlPark",
      text:"Первый блин комом, так было и так будет всегда. Первая галера она как первая любовь, даже если захочешь не забудешь." +
    ...
    {
      img:sputnik,
      data:"Сентябрь 2021 - Настоящее время",
      title:"Спутник",
      text:"Именно здесь я освоил все прелести настоящей фронтенд разработки. Первые выполненные и сорванные спринты, обучение чему-то на скорую руку, Scrum, Agile, бесконечные созвоны и митинги, все это я получил здесь. Безумно благодарен " +
          "компании и всем людям которые мне встречались на этом пути. Этот опыт незаменим."
    },
    {
      img:xakaton,
      data:"Декябрь 2023",
      title:"Хакатон Skillfactory x ТПУ",
      text:"Увидев объявление о запуске хакатона по веб-разработке я был крайне удивлен, а так как я уже получил какой-то опыт в разработке приложений стало интересно попробовать свои силы в соревновании. Взяв на себя роль менеджера, аналитика, дизайнера и фронтенд разработчика, за неделю с командой сделали качественную и красивую электронную сервисную книжку, за что и забрали первое место в хакатоне."
    },
    // {
    //   img:education,
